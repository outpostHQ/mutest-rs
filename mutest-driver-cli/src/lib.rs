...
pub fn command(name: &'static str) -> clap::Command {
    let cmd = clap::Command::new(name)
        ...
        .arg(clap::arg!(--"filter-mutations" [MUTATION_FILTERS]).value_delimiter(',').action(clap::ArgAction::Append)
            .help("\
                Filter mutations to only the ones specified by any of the filters. Multiple may be specified, either separated by commas or by repeating the option. The following filters are available:\n\
